...
import { Bot, Send, Clock, Trash2, Zap, TrendingUp, Wallet, Timer, Eye, EyeOff, Key, CheckCircle, AlertCircle, User, Volume2, Square } from 'lucide-react';

const AI_WELCOME_MESSAGES = [
  {
    role: 'ai',
    content: '👋 您好！我是 MetaBank AI 智能助手，基于 Qwen 大语言模型技术，为您带来更聪明、更贴心的金融服务。\n\n✨ 我可以帮您：\n- 💰 查询国脉币余额\n- 📈 分析市场行情和价格预测\n- 👀 查看社区用户最近动态（如：帮我看看老张头最近在干嘛）\n- 🛒 快捷买卖操作\n- 💸 代币转账\n- ⏰ 设置自动交易任务\n\n🌟 请问您需要什么帮助？',
    tool_calls: [],
  },
];
...
function ToolResultBlock({ result }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ border: `2px solid ${ok ? '#fcd34d' : '#fca5a5'}`, background: ok ? 'linear-gradient(135deg, #fffbeb 0%, #fff7ed 100%)' : 'linear-gradient(135deg, #fef2f2 0%, #fff1f2 100%)' }}>
      <div style={{ padding: '1rem 1.25rem', borderBottom: ok ? '1px solid #fed7aa' : '1px solid #fecaca' }}>
        <div className="flex items-center gap-2">
          <span style={{ fontSize: '1.125rem', fontWeight: 600, color: ok ? '#ea580c' : '#dc2626' }}>{label}</span>
      ...
      <div style={{ padding: '1rem 1.25rem', fontSize: '1rem' }}>
        ...
        {ok && data.balance !== undefined && <p style={{ fontSize: '1.25rem', fontWeight: 700, color: '#ea580c' }}>{data.balance?.toLocaleString()} GMC</p>}
      </div>
    </div>
  );
}
...
export default function AIAssistant() {
  ...
  const [savedUserKey, setSavedUserKey] = useState(() => localStorage.getItem('user_dashscope_key') || '');
  const msgEnd = useRef(null);
  const isActiveRoute = location.pathname === '/app/ai';
  const { messages, loading, send, speaking, speak, stop } = useChatStream({
    mode: null,
    isActive: isActiveRoute,
    autoSpeakFirst: autoSpeak,
    initialMessages: AI_WELCOME_MESSAGES,
  });
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2" style={{ marginTop: '0.25rem' }}>
              {isQwenActive ? <span className="flex items-center gap-1" style={{ fontSize: '1.0625rem', color: '#16a34a' }}><CheckCircle size={14} />Qwen {llmStatus?.model || 'qwen3.6-flash'} 已启用</span> : <span className="flex items-center gap-1" style={{ fontSize: '1.0625rem', color: '#6b7280' }}><AlertCircle size={14} />本地模式（未配置 API Key）</span>}
            </div>
          </div>
        </div>
      ...
      {tab === 'chat' && (
        <div className="flex flex-col h-[calc(100vh-14rem)]">
          ...
          <div className="flex mt-4" style={{ gap: '0.75rem' }}>
            <input className="input-field flex-1" placeholder="向AI助手提问... 例如：帮我分析国脉科技的走势" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendMessage()} style={{ fontSize: '1.0625rem' }} />
            <button onClick={sendMessage} className="btn-primary px-4"><Send size={18} /></button>
          </div>
        </div>
      )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3" style={{ gap: '1rem' }}>
              <div><label className="block mb-1" style={{ fontSize: '1rem', color: '#6b7280' }}>任务类型</label><select className="input-field" value={taskForm.task_type} onChange={e => setTaskForm({...taskForm, task_type: e.target.value})} style={{ fontSize: '1.0625rem' }}><option value="dca">定时定投 (DCA)</option><option value="price_trigger">价格触发</option><option value="timed">定时加仓</option></select></div>
              <div><label className="block mb-1" style={{ fontSize: '1rem', color: '#6b7280' }}>交易标的</label><select className="input-field" value={taskForm.symbol} onChange={e => setTaskForm({...taskForm, symbol: e.target.value})} style={{ fontSize: '1.0625rem' }}><option value="GM">国脉科技</option><option value="GMAI">国脉AI概念</option><option value="GMC">国脉币</option><option value="GMFT">国脉币期货</option><option value="METAV">元宇宙指数</option><option value="AIFIN">AI金融ETF</option></select></div>
              <div><label className="block mb-1" style={{ fontSize: '1rem', color: '#6b7280' }}>操作</label><select className="input-field" value={taskForm.action} onChange={e => setTaskForm({...taskForm, action: e.target.value})} style={{ fontSize: '1.0625rem' }}><option value="buy">买入</option><option value="sell">卖出</option></select></div>
              <div><label className="block mb-1" style={{ fontSize: '1rem', color: '#6b7280' }}>数量（份）</label><input type="number" className="input-field" placeholder="100" value={taskForm.amount} onChange={e => setTaskForm({...taskForm, amount: e.target.value})} style={{ fontSize: '1.0625rem' }} /></div>
              {taskForm.task_type === 'price_trigger' && <div><label className="block mb-1" style={{ fontSize: '1rem', color: '#6b7280' }}>目标价格</label><input type="number" step="0.01" className="input-field" placeholder="11.5" value={taskForm.target_price} onChange={e => setTaskForm({...taskForm, target_price: e.target.value})} style={{ fontSize: '1.0625rem' }} /></div>}
              <div><label className="block mb-1" style={{ fontSize: '1rem', color: '#6b7280' }}>间隔（分钟）</label><input type="number" className="input-field" placeholder="60" value={taskForm.interval_minutes} onChange={e => setTaskForm({...taskForm, interval_minutes: e.target.value})} style={{ fontSize: '1.0625rem' }} /></div>
              <div><label className="block mb-1" style={{ fontSize: '1rem', color: '#6b7280' }}>重复次数</label><input type="number" className="input-field" placeholder="5" value={taskForm.repeat} onChange={e => setTaskForm({...taskForm, repeat: e.target.value})} style={{ fontSize: '1.0625rem' }} /></div>
            </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {autoTasks.map(t => <div key={t.id} className="card flex items-center justify-between" style={{ padding: '1.25rem' }}><div className="flex items-center gap-3"><div className={`w-12 h-12 rounded-xl flex items-center justify-center ${t.status === 'active' ? 'bg-green-500/15 text-green-400' : 'bg-gray-50 text-gray-400'}`}><Clock size={24} /></div><div><p className="font-medium" style={{ fontSize: '1.0625rem' }}>{t.task_type === 'dca' ? '定投' : t.task_type === 'price_trigger' ? '价格触发' : '定时'} {t.action === 'buy' ? '买入' : '卖出'} {t.symbol} {t.amount}份</p><p style={{ fontSize: '1.0625rem', color: '#6b7280' }}>每{t.interval_minutes}分钟 · 已执行{t.executed}/{t.repeat}次{t.target_price ? ` · 目标价: ${t.target_price}` : ''}</p></div></div><div className="flex items-center gap-2"><span className={`px-2.5 py-1 rounded-full ${t.status === 'active' ? 'bg-green-500/15 text-green-400' : 'bg-gray-100 text-gray-400'}`} style={{ fontSize: '1rem' }}>{t.status === 'active' ? '运行中' : '已取消'}</span>{t.status === 'active' && <button onClick={() => cancelTask(t.id)} className="text-red-400/50 hover:text-red-400"><Trash2 size={16} /></button>}</div></div>)}
